...
case class SearchController(config: Configuration, searchService: SearchService, contentRenderer: ContentRenderer) {
  val QUERY_PARAM = "q"
  ...
  def apply(req: Request): Response =
    if (req.params(QUERY_PARAM) == null || req.params(QUERY_PARAM).isEmpty) {
      ...
      InputStreamResponse(Status.Ok, contentRenderer.render(results))
    }
}
